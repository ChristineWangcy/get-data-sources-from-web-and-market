...
dir = '/Users/chunyanwang/Christine documents/projects/downloaded stock data/stocks daily data - tushare/'
files = os.listdir(dir)
for f in files:
    if f[3] != '3':
    #if f[] == 'SH$601838.TXT':
        data = pd.read_table(dir + f,
                             encoding='gb2312',header=None,names=['date','open','high','low','close','volume'])
        print(data.head())
        data = data.iloc[2:-1,:]
        data['stock'] = f[3:9]
        ...
        data1.to_csv('data files/stocks weekly data with indicators/' + f +'_alldata.csv')

        #merge all_data with data1
        all_data = pd.concat([all_data,data1])
        print(f+ ' done')
        print(len(data1),len(all_data))
        all_data.to_csv('data files/allstock data in one file/weekly all data.csv')
print(all_data)